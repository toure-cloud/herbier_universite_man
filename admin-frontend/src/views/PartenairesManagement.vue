  ...
  <div class="management-page">
    ...
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <router-link to="/partenaires" class="nav-item active">
    ...
    <main class="main-content">
      <header class="top-bar">
        <div class="page-title">
          ...
          <p>Ajoutez, modifiez ou supprimez des partenaires</p>
        </div>
      ...
      <div class="filters-card">
        <div class="search-box">
          ...
          <input type="text" v-model="searchQuery" placeholder="Rechercher un partenaire...">
        </div>
        ...
        <div class="stats-badge">
          <i class="fas fa-chart-simple"></i> {{ filteredPartenaires.length }} partenaire(s)
        </div>
      </div>

      <!-- Vue Grille -->
      <div class="partners-grid">
        <div v-for="partenaire in filteredPartenaires" :key="partenaire.id" class="partner-card">
          <div class="partner-logo">
            <img :src="getFullImageUrl(partenaire.logo || partenaire.image)" @error="handleImageError" :alt="partenaire.nom">
            <div class="partner-status" :class="{ active: partenaire.actif }">
              {{ partenaire.actif ? 'Actif' : 'Inactif' }}
            </div>
          </div>
          <div class="partner-info">
            <h3>{{ partenaire.nom }}</h3>
            <p class="partner-type" v-if="partenaire.type">
              <i class="fas fa-tag"></i> {{ partenaire.type }}
            </p>
            <p class="partner-description">{{ partenaire.description || 'Aucune description' }}</p>
            <div class="partner-footer">
              <a v-if="partenaire.site_web" :href="partenaire.site_web" target="_blank" class="partner-link">
                <i class="fas fa-external-link-alt"></i> Visiter le site
              </a>
              <span class="partner-order" v-if="partenaire.ordre !== undefined">
                <i class="fas fa-sort"></i> Ordre: {{ partenaire.ordre }}
              </span>
            </div>
          </div>
          <div class="partner-actions" v-if="isSuperAdmin">
            <button @click="editItem(partenaire)" class="btn-edit">
              <i class="fas fa-edit"></i>
            </button>
            <button @click="deleteItem(partenaire)" class="btn-delete">
              <i class="fas fa-trash"></i>
            </button>
          </div>
        </div>
        <div v-if="filteredPartenaires.length === 0" class="empty-state">
      ...
      <div class="modal" :class="{ active: showModal }" @click.self="closeModal">
        <div class="modal-content">
          <div class="modal-header">
            <h2>{{ editingId ? 'Modifier' : 'Ajouter' }} un partenaire</h2>
          ...
          <form @submit.prevent="saveItem" class="modal-form">
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" v-model="form.nom" required placeholder="Nom du partenaire">
              </div>
              <div class="form-group">
                <label><i class="fas fa-tag"></i> Type</label>
                <input type="text" v-model="form.type" placeholder="Université, ONG, Institution...">
              </div>
            </div>
            <div class="form-group">
              <label><i class="fas fa-align-left"></i> Description</label>
              <textarea v-model="form.description" rows="3" placeholder="Description du partenaire"></textarea>
            </div>
            <div class="form-row">
              <div class="form-group">
                <label><i class="fas fa-globe"></i> Site web</label>
                <input type="url" v-model="form.site_web" placeholder="https://example.com">
              </div>
              <div class="form-group">
            ...
            <div class="form-group">
              ...
              <div class="image-upload-area" 
                ...
                <div v-if="form.image_preview" class="image-preview">
                  <img :src="form.image_preview" alt="Aperçu du logo">
                  <button type="button" class="remove-image" @click.stop="removeImage">✕</button>
                </div>
                <div v-else class="upload-placeholder">
                  <i class="fas fa-cloud-upload-alt"></i>
                  <p>Cliquez ou déposez un logo ici</p>
                  <span class="upload-hint">PNG, JPG, JPEG, WEBP</span>
                </div>
                <input type="file" ref="fileInput" @change="handleFileSelect" accept="image/*" style="display:none">
              </div>
              <small class="form-help" v-if="form._existing_image">Logo actuel</small>
            </div>
...
export default {
  ...
  computed: {
    ...
    typesList() {
      const types = [...new Set(this.partenaires.map(p => p.type).filter(Boolean))]
      return types.sort()
    },
    filteredPartenaires() {
  ...
  methods: {
    async loadData() {
      try {
        const res = await axios.get(`${API_BASE_URL}/api/partenaires/`, {
          headers: {
        ...
        this.showToast('error', 'Erreur lors du chargement des partenaires')
      }
    },

    getFullImageUrl(imagePath) {
      if (!imagePath) return '/src/images/logo-placeholder.png'
    ...
    handleImageError(e) {
      e.target.src = '/src/images/logo-placeholder.png'
    },
    ...
    editItem(item) {
      ...
      const existingImage = item.logo || item.image || ''
      this.form = {
        ...item,
    ...
    async saveItem() {
      ...
      try {
        ...
        if (this.form.image_file instanceof File) {
          data.logo = this.form.image_file
        } else if (this.editingId && this.form._existing_image) {
          data.logo = this.form._existing_image
        }

        let response
        if (this.editingId) {
          response = await axios.put(
            `${API_BASE_URL}/api/partenaires/${this.editingId}/`,
            data,
            {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
          ...
          this.showToast('success', 'Partenaire modifié avec succès')
        } else {
          ...
          this.showToast('success', 'Partenaire ajouté avec succès')
        }
    ...
    async deleteItem(item) {
      ...
      if (!confirm(`Supprimer définitivement le partenaire "${item.nom}" ?`)) return
      
      try {
        await axios.delete(
          `${API_BASE_URL}/api/partenaires/${item.id}/`,
          {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
        ...
        this.showToast('success', 'Partenaire supprimé avec succès')
        await this.loadData()
      } catch (error) {
...
@media (max-width: 480px) {
  ...
  .partner-actions .btn-edit,
  .partner-actions .btn-delete {
    justify-content: center;
  }
}
</style>